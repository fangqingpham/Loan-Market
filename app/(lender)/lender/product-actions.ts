...
function parseFields(formData: FormData, errorPath: string) {
  ...
  const productDescription = str(formData, "product_description");
  const importantConditions = str(formData, "important_conditions");
  ...
  return {
    ...
    product_description: orNull(productDescription),
    important_conditions: orNull(importantConditions),
  };
}
...
export async function createProductAction(formData: FormData): Promise<void> {
  ...
  revalidatePath(LIST);
  redirect(`${LIST}?message=${encodeURIComponent("Your product listing is now active.")}`);
}
...
export async function updateProductAction(formData: FormData): Promise<void> {
  ...
  const patch: ListingUpdate = parseFields(formData, editPath);
